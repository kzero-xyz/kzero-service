...
/**
 * Download zkLogin proof assets for proof-worker
 *
 * This script downloads the zkLogin.zkey file (588MB) from the Sui Foundation
 * to apps/proof-worker/assets/. It can be run before pnpm install to avoid
 * postinstall issues.
 *
 * Usage:
 *   node scripts/download-proof-worker-assets.mjs
 */
...
// Target directory: apps/proof-worker/assets (from project root)
const ASSETS_DIR = join(__dirname, '..', 'apps', 'proof-worker', 'assets');
...
async function downloadAssets() {
  console.log('🔧 Checking zkLogin assets for proof-worker...\n');
